 ...
 * 월별 서비스 비용 조회
 */
async function getMonthlyServiceCosts() {
    const now = new Date();
 ...
 * 비용 최적화 제안
 */
async function getCostOptimizationRecommendations() {
 ...
 * 비용 이상 징후 감지
 */
function detectCostAnomalies(costData, dailyCosts) {
    const alerts = [];
    // 월간 비용 급증 감지
    ...
    // DynamoDB 비용 특별 모니터링
    const dynamoService = costData.services.find((s) => s.name.includes('DynamoDB'));
...
//# sourceMappingURL=cost-breakdown.js.map