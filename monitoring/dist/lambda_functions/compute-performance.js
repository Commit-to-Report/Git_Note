 ...
 */
async function getECSServiceStatus() {
    const services = [];
    try {
        // ECS 클러스터 확인
...
async function getPerformanceMetrics() {
    ...
    try {
        ...
        // 데이터 포인트 정렬 및 포맷팅
        const dataPoints = cpuMetrics.Datapoints?.sort((a, b) => new Date(a.Timestamp).getTime() - new Date(b.Timestamp).getTime()) || [];
        ...
        // Memory 데이터 매핑
        const memoryDataPoints = memoryMetrics.Datapoints?.sort((a, b) => new Date(a.Timestamp).getTime() - new Date(b.Timestamp).getTime()) || [];
 ...
 */
async function getServiceUtilization(serviceName) {
    const endTime = new Date();
 ...
 * 성능 이상 징후 감지
 */
function detectPerformanceIssues(services, metrics, responseMetrics) {
    const alerts = [];
    // 서비스 상태 확인
    services.forEach(service => {
        ...
        if (service.deployments) {
            const failedDeployments = service.deployments.filter((d) => d.status === 'FAILED');
...
//# sourceMappingURL=compute-performance.js.map