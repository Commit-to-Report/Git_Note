...
exports.handler = async (event) => {
    ...
    try {
        ...
        console.log(`[AutoReportHandler] Found ${activePresets.length} active presets for ${frequency}`);
        ...
        for (const preset of activePresets) {
            try {
                ...
                if (!repository) {
                    console.log(`[AutoReportHandler] Skipping ${userId} - no repository configured`);
                    continue;
                }

                if (!accessToken) {
                    console.log(`[AutoReportHandler] Skipping ${userId} - no access token`);
                    continue;
                }

                console.log(`[AutoReportHandler] Generating report for ${userId} (${repository})`);
                ...
                if (success) {
                    successCount++;
                    console.log(`[AutoReportHandler] Successfully generated report for ${userId}`);
                } else {
                    failCount++;
...
async function scanActiveUserPresetsByFrequency(frequency) {
    const params = {
        ...
        FilterExpression: 'autoReportEnabled = :enabled AND reportFrequency = :frequency',
 ...
 */
async function callBackendApiToGenerateReport({ accessToken, repository, since, until, reportStyle, userId }) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error(`[AutoReportHandler] Backend API error: ${response.status}`, responseData);
            return false;
        }
    } catch (error) {
